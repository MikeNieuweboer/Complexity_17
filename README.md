...
## Files
src/ea.py: optimizes the weights of the neural network through the use of evolutionary algorithms. NO LONGER WORKS AND HAS BEEN REPLACED BY src/train.py.
src/mcaf.py: simulates the destruction of the grid and generates the data used in the presentation. Use --help to find out CLI use.
src/plot_mcaf.py: plots the data generated by src/mcaf.py. Use --help to find out CLI use.
src/utils.py: utility file containing general functions and variables for loading and reading weights and images. Can be run to test out the image loading functionality.
