...
- plot_mcaf.py -> plots the data generated by src/mcaf.py. Use --help to find out CLI use.
  - **To generate the plots in the slide, see [4.4. Example Usage (MCAF plots)](#44-example-usage-mcaf-plots)**
- plotting.py -> contains two functions to plot or animate heatmaps of gridstates. If ran will output animation of star weights in data/.
...
## 4.4. Example Usage (MCAF plots)
To generate the data used in the mcaf plots, the `mcaf.sh` file can be used, but given its long runtime, it is advised to just use the data already generated and placed in the data folder. This data can be plotted using the `plot_mcaf.py` file, as such:
> uv run src/plot_mcaf.py Gr50-Ch8-Hi64_star blob

> uv run src/plot_mcaf.py Gr50-Ch8-Hi64_star random
