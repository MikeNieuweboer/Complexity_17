...
class NN(nn.Module):
    ...
    def __init__(self, num_channels, hidden_layer_size):
        ...
        self.hidden_layer = nn.Conv2d(in_channels=3*num_channels, out_channels=hidden_layer_size, kernel_size=1)
        self.output_layer = nn.Conv2d(in_channels=hidden_layer_size, out_channels=num_channels)

        nn.init.zeros_(self.output_layer.weight)
        nn.init.zeros_(self.output_layer.bias)
        
    def percieve(self, state_grid):
        ...
        >>> perception = net.percieve(state_grid)
        >>> perception.shape
        ...
        sobel_x = torch.tensor([[-1, 0, 1], 
                                [-2, 0, 2], 
                                [-1, 0, 1]],
                                dtype=torch.float32).view(1,1,3,3).repeat(M,1,1,1)
        ...
        # create the layer
        conv_x = nn.Conv2d(M, M, 3, padding=1, padding_mode="circular", groups=M, bias=False)
        conv_y = nn.Conv2d(M, M, 3, padding=1, padding_mode="circular", groups=M, bias=False)
        ...
        perception_grid = torch.cat([sobel_x_output, sobel_y_output, input_tensor], dim=1)
    ...
    def forward(self, state_grid):
        ...
        """
        perception_grid = self.percieve(state_grid) # (1, 3*C, H, W)
